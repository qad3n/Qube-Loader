...
// is a non creature object (door / lever / campfire), so the target address is then 0.
...
typedef struct CubeSelection
{
    ...
    int32_t typeByte; // raw target discriminator (+0x140): 0x80 to 0x82 container, 0x83 dialog,
...
typedef struct CubeCamera
{
    ...
    float distance; // third person zoom
