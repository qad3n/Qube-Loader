...
namespace cube
{
    ...
    public:
        ...
        bool isCompanionFood() const
        {
            return isFood();
        } // taming feeds a Food item to a tameable critter (produces a Companion)
        // Coin value via the game price function (this is the buy price). Needs an api bound item.
        int getValue() const
        {
            return (m_api && m_data.address) ? static_cast<int>(m_api->items.value(m_api, m_data.address))
                                             : 0;
        }
        ...
        bool set(ItemField field, double value) const
        {
            return m_api && m_data.address &&
                   m_api->items.setField(m_api, m_data.address, static_cast<int32_t>(field), value) != 0;
        }
