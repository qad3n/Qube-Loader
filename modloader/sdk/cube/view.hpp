...
namespace cube
{
    ...
    public:
        ...
        bool refresh()
        {
            m_valid = m_api && m_api->camera.get(m_api, &m_data) != 0;
            return m_valid;
        }
        ...
        bool set(CameraField field, double value) const
        {
            return m_api && m_api->camera.set(m_api, static_cast<int32_t>(field), value) != 0;
        }
    ...
    public:
        ...
        bool refresh()
        {
            m_valid = m_api && m_api->display.get(m_api, &m_data) != 0;
            return m_valid;
        }
        ...
        bool set(DisplayField field, int value) const
        {
            return m_api && m_api->display.set(m_api, static_cast<int32_t>(field), value) != 0;
        }
        ...
        bool setResolution(int x, int y) const
        {
            return set(DisplayField::ResolutionX, x) && set(DisplayField::ResolutionY, y);
        }
    ...
    public:
        ...
        bool refresh()
        {
            m_valid = m_api && m_api->audio.get(m_api, &m_data) != 0;
            return m_valid;
        }
        ...
        bool playSoundAt(int soundId, const Vec3& at, float volume = kFullVolume,
                         float pitch = kNormalPitch) const
        {
        ...
        bool setMusicVolumeLive(float volume) const
        {
            return m_api && m_api->audio.setMusicVolume(m_api, volume) != 0;
        }
